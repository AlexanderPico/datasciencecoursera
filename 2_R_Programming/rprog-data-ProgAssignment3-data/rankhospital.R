rankhospital <- function(state, outcome, num = "best") {
    ...
    if(num=="best"){num<-1}
    else if(num=="worst"){num<-nrow(stateData)} #only works AFTER removing NA rows
    if(num>nrow(stateData)){return(NA)}
    
    stateData[order(stateData[,outcomeIndex],stateData[,2])[num],2] #hospital name for min outcome
